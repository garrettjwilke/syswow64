...
@export_range( 0.0, HOURS_IN_DAY, 0.0001 ) var day_time : float = 0.0 :
	set( value ) :
		...
		if day_time < 0.0 :
			...
			day_time -= HOURS_IN_DAY
			day_of_year += 1
			var day_number = "%.02f" % day_of_year
			emit_signal("total_day_cycles", day_of_year)
		_update()
...
func _on_day_speed_slider_value_changed(value):
	time_scale = value
	print("time scale: ", value)
